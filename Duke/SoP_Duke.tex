...
\textbf{Foundations in Embedded Systems and Robotics.}
My interest in robustness and systems behavior started with \textbf{embedded systems and robotics} rather than datacenters. On an \textbf{origami-inspired soft robotic arm} platform that will appear in an ICRA 2025 workshop, and on earlier autonomous robots and racing systems, I helped design and debug the low-level control stack: \textbf{CAN bus} networks linking ECUs and sensors, \textbf{I\textsuperscript{2}C} and \textbf{SPI} links to range finders and IMUs, and \textbf{UART} links to higher-level controllers. I implemented firmware on microcontrollers such as \textbf{STM32} and Arduino-class boards, wrote interrupt-driven drivers, and tracked down timing and noise issues on the bus with oscilloscopes and logic analyzers. These projects taught me to think carefully about \textbf{timing, jitter, and failure modes} at the hardware and protocol level, and to design software that remains robust when devices misbehave or messages are delayed or dropped. Although my Ph.D.\ focus has shifted from robotics to core systems, this experience with low-level communication and real-world constraints now informs how I approach \textbf{accelerator- and latency-sensitive systems} in the datacenter.

\textbf{Distributed Reliability and Agentic-DS-Ops.}
Building on this cross-layer background, I moved toward \textbf{distributed reliability} through my current \textbf{``agentic-ds-ops''} project with Prof.\ Ryan Huang at the University of Michigan. We started from a simple question: how can a \textbf{ZooKeeper-based coordination service} become more observable and automatically controllable in the presence of subtle performance faults? On CloudLab, I deployed ZooKeeper and HDFS clusters behind HAProxy, instrumented them with Prometheus, and injected gray failures using tools like ChaosBlade. This setup let us see how degradations show up in metrics, such as persistent queue buildup or unusual correlations between CPU and tail latency.
