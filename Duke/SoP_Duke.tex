...
\textbf{Fit with Duke.}
I see a fit with \textbf{Prof.~Danyang~Zhuo}'s \textbf{Harmonic}~\cite{295657}, which studies performance isolation for high-speed RDMA.
My work on \emph{CUDA Proxy Player} trained me to diagnose multi-tenant interference end-to-end and to design runtime hooks that expose when orchestration overhead or resource contention dominates tail latency; I hope to contribute a measurement-driven methodology to Harmonic by building stress workloads, defining isolation metrics, and adding stability/``never-worsen'' guardrails for isolation decisions.
I am also excited about \textbf{Prof.~Xiaowei~Yang}'s \textbf{CableMon}~\cite{246308}, which infers network health from noisy physical-layer signals.
My telemetry-to-action ZooKeeper mitigation work---mapping anomaly signals under partial observability to a constrained action set with validation gates---aligns closely with CableMon's signal-to-decision pipeline, and I hope to help translate inferred signals into actionable, safe operational policies.
