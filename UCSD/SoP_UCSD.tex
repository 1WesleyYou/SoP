...
I am applying to the PhD program in Computer Science at \school\ to work on reliable systems at the intersection of operating systems, distributed systems, and ML infrastructure, with a particular interest in why complex production systems fail in subtle ways and how we can make them more predictable, diagnosable, and mitigatable.

\textbf{Path into Systems, Networking, and ML Infrastructure:}
Before moving fully into systems research, I spent several years working on embedded systems and robotics, which taught me that failures often stem more from unreliable data and weak fault handling than from the control law itself. As an embedded developer for an autonomous sentry robot in \textbf{RoboMaster} competitions and later a research assistant in \textbf{UMich's HDRLab} advised by \textbf{Prof.~Xiaonan~Huang} on a modular soft robot, I was responsible both for firmware that connected MCUs, sensors, and actuators over CAN and $I^2C$ and for the model-based dynamics and control code that consumed these streams; our HDRLab project received a \textbf{best-poster award} at an \textbf{ICRA 2025 workshop} \cite{you2025origami}. In practice, however, the limiting factor was often not the controller design but the quality and robustness of the data path: sensor readings could be noisy, delayed, or intermittently corrupted. To keep the system safe and usable, I designed a small but explicit metrics pipeline with filtering, sanity checks, and guards, and implemented fallbacks to redundant signals or predefined degraded-but-safe modes when data quality dropped or components failed. Working under tight power and timing constraints, I had to design controllers that used limited actuation budget efficiently while still enforcing strong safety and fault-tolerance guarantees. Seeing how these reliability concerns cut across hardware, control, and software is what motivated me to pursue operating systems, distributed systems, and ML infrastructure as a coherent stack, and to think of myself as a systems student rather than only a robotics student.

\textbf{Research Preparation: From Embedded Reliability to Distributed Systems and ML Runtimes:}
These early experiences with observability, fault containment, and graceful degradation shaped how I approach larger-scale distributed and ML systems and prepared me for subsequent research projects.

In \textbf{embedded and soft robotics}, I learned to treat sensors and actuators as imperfect, failure-prone components and to design around them. Building and debugging firmware for RoboMaster robots and HDRLab's soft arm required me to reason carefully about timing, bandwidth constraints, and failure modes on real hardware. I became comfortable tracing bugs across multiple layers---MCU code, communication buses, real-time control loops, and higher-level planning---and developed an intuition for when a system is truly stable versus barely holding together. This was my first training in reliability as a cross-cutting concern rather than a property of any single module.
...
As part of an \textbf{Advanced Operating Systems} course project and a subsequent independent follow-up, I also tackled efficiency challenges in serving launch-bounded MoE-style inference workloads. Working with one collaborator, I co-designed the \textbf{CUDA Proxy Player}, a multi-path runtime that can route each request to an eager, CUDA Graph, or persistent-kernel path depending on workload characteristics. Profiling online decoding pipelines revealed dense timelines of sub-100$\mu$s kernels separated by CPU gaps, indicating that the workload was effectively launch-bounded rather than compute-bounded. My main responsibility was to build the benchmarking and evaluation framework: constructing microbenchmarks and compact MoE forward passes that span realistic expert sizes and hidden dimensions, and using them to compare the different execution paths fairly. In this setting, our CUDA Graph implementation matches a hand-tuned fusion baseline while cutting latency by about 24\% versus naive dispatch. Beyond familiarity with CUDA streams, graphs, and persistent kernels, this project taught me how to identify the true bottleneck, design objective experiments, and choose representative real-world cases to highlight the strengths and limits of new runtime mechanisms.

\textbf{Research Interests in Reliable Operating, Distributed, and ML Systems:}
These projects collectively shaped my research interests. I am especially interested in \textbf{fail-slow} and \textbf{gray failures} in \textbf{large-scale ML training and inference}. As clusters scale to thousands of GPUs, subtle performance pathologies can quietly erode throughput and inflate tail latency without ever causing crashes. I would like to design runtime mechanisms and control planes that treat system metrics and training signals as noisy sensors for such latent failures, and adapt scheduling, checkpointing, or collective communication in response, so that clusters can detect and react before problems become visible outages.

More broadly, I am drawn to \textbf{reliability} and \textbf{efficiency} problems across the stack of systems that support \textbf{modern ML workloads}: from distributed storage and coordination services, to communication and scheduling layers for large-scale training, to runtimes and serving systems for dynamic, launch-bounded inference. Across these topics, a unifying theme is building systems that expose the right abstractions to make complex cloud and ML infrastructure more observable, diagnosable, and controllable in production. I am particularly interested in questions such as: how to design metrics and tracing infrastructures that expose the right signals for gray failures; how to integrate these signals into runtime policies that can mitigate fail-slow behavior without sacrificing utilization; and how to make these mechanisms practical in the noisy, multi-tenant settings of real cloud environments.

\textbf{Long-Term Career Goals and the Role of a PhD:}
Long term, I hope to become a systems researcher who designs robust operating systems and distributed infrastructure that cloud operators can rely on. I am motivated by the gap between sophisticated cloud platforms and the fragility we still see in practice---outages, SLO violations, and gray failures---and my realistic goal is to build well-evaluated systems that improve SLO robustness and make such incidents less frequent. A PhD experience at \school\ will allow me to move beyond course projects to formulating open research questions, building principled abstractions, and evaluating them under realistic workloads, and to learn how to connect low-level mechanisms with high-level reliability goals so that I can eventually lead research efforts that strengthen real-world cloud and ML infrastructure, whether in academia or in an industrial research lab.

textbf{Fit with UCSD CS and Prospective Advisors:}
\school's strengths in systems and ML/AI infrastructure align closely with my preparation and research interests in reliable operating, distributed, and ML systems. My work on embedded and soft-robot reliability, ZooKeeper-based agentic operations, and CUDA runtimes has trained me to build instrumentation pipelines, fault-injection testbeds, and performance-sensitive runtimes---skills that match well with \schoolShort's focus on making complex ML and cloud systems efficient, observable, and dependable.

I see a particularly strong fit with \textbf{Prof.~Yiying~Zhang} and \textbf{Prof.~Yufei~Ding}. Prof.~Zhang's work in WukLab on systems for ML/AI---from distributed prompt scheduling and autotuning for LLM serving, to far-memory prefetching, serverless runtimes, and SmartNIC-based acceleration---resonates with my interest in building reliable and efficient infrastructure for modern ML workloads. My experience designing an agentic control plane for ZooKeeper and a CUDA Graph--based runtime for launch-bounded MoE inference has given me practice in identifying end-to-end bottlenecks, building evaluation harnesses, and reasoning about tail behavior under realistic workloads; I would be excited to extend these skills to WukLab's research on ML training and serving systems, exploring, for example, how to make speculative decoding, prompt scheduling, and serverless execution more robust to fail-slow behavior and gray failures.

Prof.~Ding's PICASSO Lab, focusing on domain-specific languages, compiler and architecture co-design, and high-performance frameworks for emerging technologies such as quantum computing and machine learning, is equally appealing. The CUDA Proxy Player project taught me how much performance and reliability depend on getting the right abstractions at the compiler and runtime level: exposing the right knobs for kernel aggregation, launch scheduling, and hardware utilization can turn a launch-bounded pipeline into an efficient and predictable one. Working with Prof.~Ding, I would like to explore programming and compilation frameworks that make it easier to express and optimize the communication and computation patterns in large-scale ML workloads, and to bake in observability and robustness concerns at the language and runtime level.

Together, these groups and \school's broader systems and ML communities provide exactly the environment I am looking for to develop a coherent research agenda on reliable, diagnosable systems for modern cloud and ML workloads.
