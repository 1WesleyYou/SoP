...
As part of an \textbf{Advanced Operating Systems} course project and a subsequent independent follow-up, I studied bottlenecks in serving \textbf{launch-bounded GPU inference workloads}, with Mixture-of-Experts inference as a motivating example. Working in a group of three, I co-designed \textbf{CUDA Proxy Player}, a multi-path runtime that routes each request to an \textbf{eager, CUDA Graph, or persistent-kernel} execution path based on workload characteristics. In our experiments, we injected additional sub-100$\mu$s micro-operations into the pipeline and observed sharp increases in end-to-end latency despite similar GPU utilization, confirming that kernel-launch and host-orchestration overhead, rather than raw compute throughput, were the dominant bottlenecks. I also implemented much of the benchmarking and evaluation framework, constructing microbenchmarks and representative forward-pass graphs that span realistic expert sizes, hidden dimensions, and batch shapes to compare these execution paths under controlled traffic patterns. In this launch-bounded regime, our CUDA Graph path matches a hand-tuned fusion baseline while reducing end-to-end latency by about 24\% relative to a naive eager baseline, reinforcing my view of performance as a \textbf{runtime design} problem grounded in clear bottleneck analysis, \textbf{objective baselines}, and \textbf{representative workloads}.
...
% Industry
\textbf{Fit with Purdue CS and Prospective Advisors:}  Purdue's focus on reliable and secure systems software aligns with my goal of building dependable operating, distributed, and ML systems. I see a particularly strong fit with \textbf{Prof.~Yongle~Zhang} and also strong alignment with \textbf{Prof.~Pedro~Fonseca}. Prof. Zhang's work on production failures and fail-slow hardware faults in cloud systems~\cite{dong2025failslow} directly overlaps with my interest in gray failures and SLO-driven reliability for ML training clusters; my experience treating ZooKeeper gray failures and backlog spikes as ``sensor readings'' for latent performance faults gives me a concrete starting point for his group's work on failure detection and diagnosable system design, especially in using noisy telemetry to explain SLO violations and guide mitigation. I am also excited about Prof. Fonseca's Reliable and Secure Systems Lab, particularly work on \textbf{kernel record–replay}~\cite{10.5555/3767901.3767935} and \textbf{kernel concurrency testing}~\cite{10.1145/3600006.3613148}, and would like to explore how ideas from record–replay, isolation, and serverless checkpointing could be adapted to reliably reproduce and mitigate fail-slow anomalies in ML infrastructure; my background building load generators, fault-injection testbeds, and agentic control platforms would transfer naturally to this setting. Together, these groups and Purdue's broader systems community provide a natural home to develop a coherent research agenda on reliable, diagnosable systems for large-scale cloud and ML workloads.
%% Summary (3-4 Lines)
