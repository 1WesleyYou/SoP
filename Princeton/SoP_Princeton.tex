...
\def\header{%
	\begin{minipage}[]{120pt}
    	...
    	MSE in CS, Fall 2026 			% Area, Program, Cycle, Year
    \end{minipage}
}
...
\paragraph{Fit with Princeton's Systems Curriculum and MSE Thesis Plan.}
Princeton is an ideal place for me to deepen systems research skills in both \emph{reading} and \emph{building}.
\textbf{COS 518 (Advanced Computer Systems)} offers paper-driven coverage of core OS topics plus an implementation-heavy project, while \textbf{COS 316 (Principles of Computer System Design)} sharpens the design-and-evaluation discipline I rely on for reproducible systems work.
\textbf{For my MSE thesis}, I will unify telemetry-to-action reliability for distributed services with performance isolation for multi-tenant ML serving: \emph{how can constrained actuation interfaces with explicit guardrails improve SLOs without instability or tail regressions?}
I plan to build a reproducible prototype with telemetry-derived state abstractions, a typed safety-checked action library, and a workload/fault-injection harness to compare against hand-tuned playbooks under overload and gray failures.
%% Summary (3-4 Lines)
